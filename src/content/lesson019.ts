...
export const lesson019: Lesson = { id: "lesson-019", unitId: "unit-006", order: 19, title: "Сначала одно, затем другое", description: "Цепочка действий, явный порядок с ～てから и отдельное それから без привязки самой て-формы к прошедшему времени.", theory: lesson019Grammar.map((item) => item.explanationRu), itemIds: [...lesson019Vocabulary, ...lesson019Grammar, ...lesson019Sentences].map((item) => item.id), exerciseIds: lesson019Exercises.map((item) => item.id), estimatedMinutes: 17 };
